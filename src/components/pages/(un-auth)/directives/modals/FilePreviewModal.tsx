...
export default function FilePreviewModal({
  ...
  useEffect(() => {
    if (fileUrl) {
      ...
      const baseUrl = fileUrl.includes('?') 
        ? fileUrl.split('?')[0] 
        : fileUrl;
      ...
      if (/\.(jpeg|jpg|png|gif|webp|bmp|svg)$/i.test(urlWithoutParams)) {
        ...
        if (url.includes('image') || url.includes('photo') || url.includes('picture') || 
            url.includes('signature') || url.includes('qr_code')) {
          setFileType('image');
        ...
        <div className="flex flex-col items-center justify-center">
          ...
          <div className="mt-2 flex flex-col gap-2 items-center">
            ...
            <button
              ...
              className="text-[#355FD0] hover:underline text-sm"
            >
              Try to view as PDF
            </button>
            ...
            >
              <Dialog.Panel className="relative transform overflow-visible rounded-lg bg-white pb-4 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-4xl">
                ...
                <div className="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse px-4">
                  <button
                    type="button"
                    className="mt-3 inline-flex w-full justify-center rounded-md bg-white px-3 py-2 text-sm font-semibold text-[#355FD0] shadow-sm ring-1 ring-inset ring-[#355FD0] hover:bg-gray-50 sm:mt-0 sm:w-auto"
                    onClick={onClose}
                    ref={cancelButtonRef}
                  >
                    Close
