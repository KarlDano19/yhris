                ...
                {reviewCycle.map((s, i) => (
                  <ScrollFadeIn key={s.step} delay={i * 60}>
                    <div className="lp-light-card p-6">
                      ...
                      <h3 className="text-sm font-bold text-gray-800 mb-2">{s.phase}</h3>
                      <p className="text-sm text-gray-500 leading-relaxed mb-4">{s.description}</p>
                      <span className="text-xs font-medium px-2.5 py-1 rounded-full" style={{ background: "rgba(255,193,7,0.1)", color: "hsl(38, 92%, 38%)", border: "1px solid rgba(255,193,7,0.2)" }}>{s.duration}</span>
                    </div>
                  </ScrollFadeIn>
                ))}
              </div>
