    ...
    if (isApplicantFavorited(applicantId)) {
      removeFavoriteMutation.mutate(applicantId, {
        onSuccess: () => {
          refetchFavorites();
          toast.custom((t) => (
            <CustomToast message="Removed from favorites!" type="success" />
          ));
        },
        onError: (error: any) => {
          toast.custom((t) => (
            <CustomToast message={error?.toString() || "Failed to remove favorite."} type="error" />
          ));
        },
      });
    } else {
      addFavoriteMutation.mutate(applicantId, {
        onSuccess: () => {
          refetchFavorites();
          toast.custom((t) => (
            <CustomToast message="Added to favorites!" type="success" />
          ));
        },
        onError: (error: any) => {
          toast.custom((t) => (
            <CustomToast message={error?.toString() || "Failed to add favorite."} type="error" />
          ));
        },
      });
    }
                ...
                {selectedApplicants.size === 0 && (
                  <div className='mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg'>
                    <p className='text-sm text-blue-800'>
                      💡 <strong>Tip:</strong> Click the &ldquo;Compare&rdquo; button next to &ldquo;View Profile&rdquo; to select up to 5 applicants for comparison.
                    </p>
                  </div>
                )}
                ...
                {applicantsData && Array.isArray(applicantsData) && applicantsData.length > 0 ? (
                  ...
                  !isLoading && (
                    <div className='text-center py-8'>
                      <p className='text-gray-600'>No applicants found matching your search criteria.</p>
                    </div>
                  )
                )}
              </div>
