...
async function getEmailMonitoringDetail(filters: any) {
    try {
        ...
        if (token) {
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/applicants/talent-email-monitoring/?${searchParams}`, config);
            if (!res.ok) {
                throw res.json();
            }
            return res.json();
...
function useGetEmailMonitoringDetail(filters: any) {
    const query = useQuery(['emailMonitoringDetailCache'], () => getEmailMonitoringDetail(filters), {
...
export default useGetEmailMonitoringDetail;