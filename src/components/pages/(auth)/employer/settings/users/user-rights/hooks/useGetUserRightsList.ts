...
async function getUserRightsList(filters: any) {
  try {
    ...
    if (token) {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/user-rights/?${searchParams}`, config);
      if (!res.ok) {
        throw res.json();
      }
      return res.json();
...
function useGetUserRightsList(filters: any) {
  return useQuery({
    queryKey: ['userRightsList', filters],
    queryFn: () => getUserRightsList(filters),
    enabled: !!filters,
  });
}

export default useGetUserRightsList;
