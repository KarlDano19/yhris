...
async function editJobPost(jobPost: any, job_post_id: string) {
  try {
...
function useEditJobPostItems() {
  const query = useMutation((props: any) => editJobPost(props.jobPost, props.job_post_id));

  return query;
}

export default useEditJobPostItems;
