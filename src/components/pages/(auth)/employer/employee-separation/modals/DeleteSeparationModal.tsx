...
import { T_DeleteSepartionModal } from '@/types/globals';

export default function DeleteSeparationModal({
  refetch,
  isOpen,
  setIsOpen,
}: {
  refetch: any;
  isOpen: T_DeleteSepartionModal;
  setIsOpen: Dispatch<T_DeleteSepartionModal | null>;
}) {
  const cancelButtonRef = useRef(null);
  const { mutate, isLoading } = useDeleteSeparation();
              ...
              <Dialog.Panel className='relative transform overflow-hidden rounded-lg bg-white pb-4 text-left shadow-xl transition-all sm:my-8 w-[500px]'>
                ...
                <div className='text-xl px-20 text-center'>
                  <p className='text-xl text-gray-600 font-bold'>
                    Are you sure you want to <span className='text-red-500'>delete</span> {isOpen.name}{' '}
                    Separation?
                  </p>
                </div>
