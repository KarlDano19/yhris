...
async function getAuditLogsItems(filters: any) {
    try {
        ...
        if (token) {
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/audit-logs/?${searchParams}`, config);
            if (!res.ok) {
                throw res.json();
            }
            return res.json();
...
export default useGetAuditLogsItems;
