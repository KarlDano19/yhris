...
import ViewEmailMonitoringLogDetails from '../modals/ViewEmailMonitoringLogDetails';
import useGetEmailMonitoringLogsItems from '../hooks/useGetEmailMonitoringLogsItems';
import useGetEmailMonitoringLogFilters from '../hooks/useGetEmailMonitoringLogFilters';
...
const Content = ({ hasActiveSubscription }: { hasActiveSubscription: boolean }) => {
  const [persistedAdvancedFilter, setPersistedAdvancedFilter] = useFilterPersistence<AuditLogFilterValues>(
    'email-monitoring-advanced-filter',
  ...
  const {
    ...
    refetch: emailLogRefetch,
  } = useGetEmailMonitoringLogsItems({ ...appliedFilter, pageSize: pageSize, currentPage: currentPage });
  const {
    data: emailFilterOptions,
    isFetching: isEmailFilterFetching,
    refetch: refetchEmailFilterOptions,
  } = useGetEmailMonitoringLogFilters(); // enabled by default

  const actionOptions = useMemo(
    () => emailFilterOptions?.actions ?? [],
    [emailFilterOptions]
  );

  const userOptions = useMemo(
    () =>
      (emailFilterOptions?.users ?? []).map((user: any) => ({
  ...
  useEffect(() => {
    if (emailLogItems) {
      setEmailLogsData(emailLogItems.records);
      setPagination({
        totalPages: emailLogItems.total_pages,
        totalRecords: emailLogItems.total_records,
      });
    }
  }, [emailLogItems]);

  useEffect(() => {
    if (!isEmailLogLoading && !isEmailLogFetching && isSearching) {
      setIsSearching(false);
    }
  }, [isEmailLogLoading, isEmailLogFetching, isSearching]);
  ...
  const handleFilterOpenChange = (value: boolean) => {
    if (value && !emailFilterOptions) {
      refetchEmailFilterOptions();
    }
  ...
  const renderRows = () => {
    ...
    if (isSearching || isEmailLogLoading || isEmailLogFetching) {
      return (
        <tr>
          <td colSpan={100}>
            <div className='py-5'>
    ...
    if (emailLogsData && emailLogsData.length > 0) {
      return emailLogsData.map((item: any) => (
        <tr key={item.id} className='cursor-pointer hover:bg-gray-100' onClick={() => setOpenModal({ id: item.id, open: true })}>
          ...
          <td className='whitespace-nowrap px-3 py-5 text-sm text-gray-500'>{item.user}</td>
  ...
  return (
    <>
      <div className='flex flex-col min-h-[80vh] mb-8'>
        <h2 className='text-xl font-bold text-indigo-dye'>Email Monitoring</h2>
            ...
            <div className='flex gap-2 lg:w-1/3'>
              <div className='flex flex-row w-full items-center gap-2'>
                <input
                  ...
                  data-tooltip-content='Search for: User, Module, Subject'
            ...
            <div className='flex-1 flex justify-end'>
              <div className='relative' ref={filterPopoverRef}>
                <button
                  ...
                  disabled={isEmailFilterFetching}
                >
                  Advance Filter
                </button>
                {isFilterOpen && (
              ...
              <Menu as='div' className='relative'>
                <MenuButton className='border border-blue-600 py-2 px-3 rounded-r-md text-blue-600 text-sm font-semibold shadow hover:shadow-md focus:shadow-none disabled:opacity-50'>
