...
import useGetAnnualMedicalReportItems from '../hooks/useExportAnnualMedicalReport';
...
export default function ExportProgressModal({
  ...
  const {
    data: exportAnnualMedicalReportData,
    isLoading: isExportAnnualMedicalReportLoading,
    refetch: exportAnnualMedicalReportRefetch,
    remove: exportAnnualMedicalReportRemove,
  } = useGetAnnualMedicalReportItems(itemsFilter);

  useEffect(() => {
    exportAnnualMedicalReportRefetch();
  }, []);

  useEffect(() => {
    if (isExportAnnualMedicalReportLoading) {
  ...
  }, [isExportAnnualMedicalReportLoading]);

  useEffect(() => {
    if (exportAnnualMedicalReportData) {
      setProgress(100);
      setDownloadUrl(exportAnnualMedicalReportData);
    }
  }, [exportAnnualMedicalReportData]);

  const customCloseModal = () => {
    setIsOpen(false);
    exportAnnualMedicalReportRemove();
  };
