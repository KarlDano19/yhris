...
function Content() {
  ...
  const { data: workAccidentIlnessReportsData, isLoading: isWorkAccidentIlnessReportsLoading, refetch: workAccidentIlnessReportsRefetch } = useGetWorkAccidentIlnessReportsItems({ ...itemsFilter, pageSize: pageSize, currentPage: currentPage });
  const menuOptions = [
    {
      name: 'Export',
      action: () => {
  ...
  useEffect(() => {
    if (workAccidentIlnessReportsData) {
      workAccidentIlnessReportsData.records.map((item: any) => {
        // item.date_of_accident = Intl.DateTimeFormat('en-US').format(new Date(item.date_of_accident));
        // item.time_of_accident = Intl.DateTimeFormat('en-US').format(new Date(item.time_of_accident));
        return item;
      });
