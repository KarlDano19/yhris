...
export async function sendEmail(data: any) {
  try {
    ...
    if (data instanceof FormData) {
      ...
      // Add shc_meeting_minutes_id if provided
      if (data.shc_meeting_minutes_id) {
        formData.append("shc_meeting_minutes_id", data.shc_meeting_minutes_id.toString());
      }
    }
    ...
    if (token) {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/shc-meeting-minutes/send-email/`,
        config
      );
      if (!res.ok) {
        throw res.json();
