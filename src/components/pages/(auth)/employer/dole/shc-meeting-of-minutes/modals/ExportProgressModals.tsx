...
import useGetExportShcMeetingMinutes from '../hooks/useExportMeetingMinutes';
...
export default function ExportProgressModal({
  ...
  const {
    data: exportShcMeetingMinutesData,
    isLoading: isExportShcMeetingMinutesLoading,
    refetch: exportShcMeetingMinutesRefetch,
    remove: exportShcMeetingMinutesRemove,
  } = useGetExportShcMeetingMinutes(itemsFilter);

  useEffect(() => {
    exportShcMeetingMinutesRefetch();
  }, []);

  useEffect(() => {
    if (isExportShcMeetingMinutesLoading) {
  ...
  }, [isExportShcMeetingMinutesLoading]);

  useEffect(() => {
    if (exportShcMeetingMinutesData) {
      setProgress(100);
      setDownloadUrl(exportShcMeetingMinutesData);
    }
  }, [exportShcMeetingMinutesData]);

  const customCloseModal = () => {
    setIsOpen(false);
    exportShcMeetingMinutesRemove();
  };
