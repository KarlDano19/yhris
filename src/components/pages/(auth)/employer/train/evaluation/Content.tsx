...
const Content = () => {
  ...
  useEffect(() => {
    if (!window.location.hash) {
      window.location.hash = 'evaluation-table';
    }
  ...
  useEffect(() => {
    if (dataBenefits) {
      dataBenefits.benefits?.map((benefit: any) => {
        benefit.date = Intl.DateTimeFormat('en-US').format(new Date(benefit.date));
        return benefit;
      });
    ...
    <>
      {currentTab === '#evaluation-table' && (
        <>
          ...
          <CreateEvaluationTabModal isOpen={isModalOpen} setIsOpen={setIsModalOpen} />
        </>
      )}
      {currentTab === '#create-evaluation-form' && (
        <>
          <div className='mx-auto max-w-5xl px-4 sm:px-6 lg:px-8'>
            <div className='flex p-4'>
              <Link href='/train/evaluation' className='flex-none flex gap-3 items-center hover:bg-gray-200'>
                <ArrowLeftIcon className='h-5 w-5' />
                <h4>Evaluation</h4>
              </Link>
            </div>
            ...
            <div className='flex space-x-9 pt-4 justify-end'>
              <button
                type='button'
