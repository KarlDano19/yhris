...
function useGetEvaluationHistoryDetails(evaluation_form_id: number | null) {
    const query = useQuery(["evaluationHistoryDetailsCache", evaluation_form_id], () => getEvaluationHistoryDetails(evaluation_form_id), { enabled: !!evaluation_form_id, refetchOnWindowFocus: false, keepPreviousData: true });
    return query;
}
