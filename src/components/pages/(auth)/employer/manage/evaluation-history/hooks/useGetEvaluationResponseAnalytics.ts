...
async function getEvaluationResponseAnalytics(
  ...
  try {
    if (!evaluation_template_id) {
      return {
        ...
        frequently_evaluated_employees: [],
        total_records: 0,
        total_pages: 1,
        current_page: 1,
        page_size: 5
    ...
    const url = `${process.env.NEXT_PUBLIC_API_URL}/api/evaluation-templates/${evaluation_template_id}/responses/analytics/${queryParams.toString() ? `?${queryParams.toString()}` : ''}`;
    ...
    // Backend now returns { analytics_chart_data: [...], frequently_evaluated_employees: [...], total_records, total_pages, current_page, page_size }
    return {
      analytics_chart_data: data?.data?.analytics_chart_data || data?.analytics_chart_data || [],
      frequently_evaluated_employees: data?.data?.frequently_evaluated_employees || data?.frequently_evaluated_employees || [],
...
export function useGetEvaluationResponseAnalytics(
  evaluation_template_id: number | null,
  ...
  const query = useQuery(
    ...
    () => getEvaluationResponseAnalytics(evaluation_template_id, pagination),
    {
...
export default useGetEvaluationResponseAnalytics;
