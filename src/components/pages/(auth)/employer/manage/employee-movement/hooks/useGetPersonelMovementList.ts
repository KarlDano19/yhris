...
async function getPersonelMovementList(filters: any) {
try {
    ...
    const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/personnel-movements/?${searchParams}`,
        config
    );
    if (!res.ok) {
        throw res.json();
...
export default useGetPersonelMovementList;