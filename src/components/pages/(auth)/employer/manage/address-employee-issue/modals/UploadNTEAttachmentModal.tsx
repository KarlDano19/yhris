...
function UploadEmployeeIssueAttachmentModal({ isOpen, setIsOpen }: { isOpen: any; setIsOpen: Dispatch<any> }) {
  const cancelButtonRef = useRef(null);
  const [toSaveData, setToSaveData] = useState<{ nte_attachment?: File }>();
  ...
  const uploadOnChange = ({ target }: { target: any }) => {
    ...
    if (file.size <= 5000000) {
      setToSaveData({ nte_attachment: file });
    } else {
              ...
              >
                <Dialog.Panel className='relative transform overflow-visible rounded-lg bg-white pb-4 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-4xl'>
                  <div className='flex bg-savoy-blue p-2 items-center'>
                    <h3 className='flex-1 text-white ml-2 font-semibold'>Upload NTE Attachment</h3>
...
export default UploadEmployeeIssueAttachmentModal;
