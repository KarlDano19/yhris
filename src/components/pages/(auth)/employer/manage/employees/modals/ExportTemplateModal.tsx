...
export default function ExportTemplateModal({
  ...
  const {
    ...
    isLoading: isExportTemplateLoading,
    refetch: exportTemplateRefetch,
    remove: exportTemplateRemove,
  } = useGetExportDummyEmployeeData(itemsFilter);

  useEffect(() => {
    exportTemplateRefetch();
  }, []);

  useEffect(() => {
    if (isExportTemplateLoading) {
  ...
  useEffect(() => {
    if (exportTemplateData) {
      setProgress(100);
      setDownloadUrl(exportTemplateData);
    }
  }, [exportTemplateData]);

  const customCloseModal = () => {
    setIsOpen(false);
    exportTemplateRemove();
  };
