...
import useDeleteProspectiveClient from '../hooks/useDeleteProspectiveClient';
...
export default function DeleteProspectiveClientModal({
  refetch,
  isOpen,
  setIsOpen,
}: {
  ...
  const { mutate, isLoading } = useDeleteProspectiveClient();

  const handleDelete = () => {
    mutate(isOpen.id, {
      onSuccess: (res: any) => {
        toast.custom(() => <CustomToast message={res.message || 'Prospective client deleted successfully.'} type="success" />, { duration: 4000 });
        setIsOpen(null);
        refetch();
      },
      onError: (err: any) => {
              ...
              <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-white pb-4 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-md">
                ...
                <div className="px-4 pt-5 pb-4">
                  <p className="text-sm text-gray-600">
                    ...
                    <span className="font-semibold text-gray-900">{isOpen?.company_name}</span>? This action cannot be undone.
                  </p>
                </div>
                <hr />
