...
import useVouchersItems from './hooks/usGetVouchers';
...
const Content = () => {
  ...
  const { data: dataVouchers, isLoading: isGetVouchersLoading, refetch } = useVouchersItems();
  ...
  const handleDelete = () => {
    // Logic to delete the voucher with selectedVoucherId
      ...
      return vouchers.map((item: any) => (
        <tr key={item.id}>
          ...
          <td className='whitespace-nowrap px-3 py-5 text-sm text-gray-500 text-center'>
            <input
                type="checkbox"
                checked={item.redeemed} // Check the checkbox if item.redeemed is true
                disabled // Disable the checkbox if you don't want it to be editable
            />
          </td>
          <td className='whitespace-nowrap px-3 py-5 text-sm text-gray-500 text-center'>
            <span onClick={() => openRedemptionModal(item.id)} className='cursor-pointer text-blue-600 underline'>{item.redemption_count?.length}</span>
          </td>
          <td className='whitespace-nowrap px-3 py-5 text-sm text-gray-500'>
           {item.redemption_from} - {item.redemption_to}
          </td>
          <td className='whitespace-nowrap px-3 py-5 text-sm text-gray-500 text-center'>
            <div className='flex space-x-2'>
                <button onClick={() => openEditVoucherModal(item)}><EditIcon /></button>
                <button onClick={() => openDeleteVoucherModal(item)}><DeleteIcon /></button>
            </div>
          </td>
        </tr>
      ));
        ...
        <tr>
          <td colSpan={7}>
            ...
            <h4 className='text-center text-gray-300 text-sm mb-4'>
              Please click create to add vouchers.
            </h4>
  ...
  return (
    <>
          ...
          <div className='mt-8 flow-root'>
            <div className='-mx-4 -my-2 overflow-x-auto sm:-mx-6 lg:-mx-8'>
              <div className='min-w-full py-2 sm:px-6 lg:px-8'>
                ...
                <hr />
                <p className='text-xs text-gray-500 mt-2'>Total record/s: {vouchers?.length}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <RedemptionCountModal 
        ...
        voucherDetails={selectedVoucherDetails}
      />
      <DeleteVoucherModal
