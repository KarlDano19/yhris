...
const DragDrop = ({ setValue }: { setValue?: Function }) => {
  ...
  const { getRootProps, getInputProps } = useDropzone({
    onDrop: (acceptedFiles: any) => {
      ...
      setFiles([...files, ...(droppedFiles.filter((file:any) => file.size <= 5000000))]);
    },
  });
    ...
    <section className='container'>
      <div
        {...getRootProps({
          className: `${
            files.length > 0 && 'p-4'
          } min-h-fit border-[1px] border-[#ACB9CB] rounded-lg`,
        })}
      >
        <input {...getInputProps()} />
        {files.length === 0 ? (
          <p className='text-center my-12 text-[#CCD8EA] font-bold'>
