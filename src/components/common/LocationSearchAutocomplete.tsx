...
const LocationSearchAutocomplete: React.FC<LocationSearchAutocompleteProps> = ({
  value,
  onChange,
  ...
  return (
    <div className="relative w-full">
      <div className="flex items-center justify-around rounded-md p-3 shadow-sm ring-1 ring-inset ring-gray-300 focus-within:ring-2 focus-within:ring-black w-full">
        <label htmlFor="location-search" className="font-semibold text-indigo-dye text-sm">
          Where
        </label>
        <input
          ref={inputRef}
      ...
      {value && value.length >= 2 && !hideTooltip && !showSuggestions && (
        <div className="absolute left-0 top-full mt-1 z-10 bg-white border border-gray-300 rounded-md px-3 py-2 text-sm text-gray-700 text-center w-full">
          Press <span className="font-bold">Enter</span> to show location suggestions
        </div>
      )}
      ...
      {showSuggestions && shouldShowAutocomplete && (() => {
          ...
          return (
            <div className="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-md shadow-lg">
              <div className="px-3 py-4 text-center">
                ...
                <p className="text-sm text-gray-600 mt-2">Searching locations...</p>
              </div>
            </div>
          );
        }
        ...
        if (value && value.length >= 2 && suggestions.length === 0) {
          return (
            <div className="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-md shadow-lg">
              <div className="px-3 py-4 text-center">
                <p className="text-sm text-gray-600">No location suggestions found for "{value}"</p>
              </div>
            </div>
          );
        }
