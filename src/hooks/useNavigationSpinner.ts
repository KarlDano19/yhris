...
function useNavigationSpinner(): { visible: boolean } {
  ...
  useEffect(() => {
    ...
    // Double-patch guard: only the first invocation (iOwnThePatch = true) applies the patch.
    // StrictMode's second invocation finds the symbol already set and skips patching.
    ...
    // Non-owner path (defensive fallback — only reachable if two instances coexist):
    // refresh the handler closure without re-patching.
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // Issue 1 fix: clear pending timer so the callback never fires on orphaned refs.
