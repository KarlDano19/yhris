...
Based on this, return a JSON object with these exact fields. All fields are required and must never be empty strings:
{
  ...
  "companyIntel": <3-5 sentences about the CLIENT company (${data.companyName || 'the company listed'}): what they do, industry, estimated size, and context relevant to a ${serviceLabel} sale. Use search results if available; otherwise use general knowledge. If any revenue or financial figures appear in USD, convert them to PHP at 56 PHP per 1 USD. Never leave this empty.>,
  "personIntel": <${isPartner
    ? `2-3 sentences describing this as a PARTNER booking. State clearly that ${data.firstName} ${data.lastName} is a rep from ${partnerName} — a YAHSHUA Partner — who booked this demo on behalf of their client (${data.companyName || 'the listed company'}). Note what this means for the sales approach: this is a partner-led deal, coordinate with the ${partnerName} rep, treat the client company as the end buyer. Do not describe this person as a direct prospect. Always write "YAHSHUA Partner" (never "YAHSHUA HRIS partner") when describing this partnership.`
