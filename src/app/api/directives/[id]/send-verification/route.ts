...
export async function POST(
  ...
  try {
    ...
    const { emailIndex } = body;

    if (emailIndex === undefined || emailIndex === null) {
      return NextResponse.json({ error: 'emailIndex is required' }, { status: 400 });
    }
    ...
    const verifyRes = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/directives/${params.id}/send-verification/`,
      {
        method: 'POST',
